    ...
    <v-container fluid grid-list-md>
        <v-layout row justify-space-around class="pt-3">
            <v-flex class="text-xs-center" xs2>
                <img   src="../../../assets/imagenes/visual/carlos1.png" width="100%">
            </v-flex>
        ...
        <v-layout row justify-space-around class="pt-3">
            <draggable  class="layout wrap list-group" :sort="false" :list="list2" group="people">
                <v-flex xs2  v-for="element in list2" :key="element.id">                        
                    <img :src="getImgUrl(element.img)" width="70%"/>
                </v-flex>
            </draggable>
        </v-layout>
    </v-container>
...
export default {
    ...
    data(){
        return {
            ...
            controlOnStart: true,
            result_exercise1: this.$route.params.resultado1,
    ...
    computed: {
        observable(){
            ...
            if(vm.list1.length == 5){
                ...
                result = amount == 5 ? 1:0;
                //vm.$router.push({ path: `/testvisual3/${vm.result_exercise1}/${result}`});
                vm.$router.push({ path: `/testvisualcompleto3/`,
                    query: {
                        ...
                        valor_ejercicio2_visual: {
                            punteo: result,
                            datos_ejercicio2: vm.list1
                        }
                    } 
                });
            }
    ...
    created(){
        let vm = this;
        console.log("OBTIENEN LOS RESULTADOS DEL EJERCICIO ANTERIOR");
        console.log(vm.valor_ejercicio1);
        vm.playSound('diferentes_carlos.mp3');
    },
    methods:{
        getImgUrl(pic) {
