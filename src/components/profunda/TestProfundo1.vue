    ...
    <v-container>
        ...
        >
            <v-layer ref="layer">
                ...
                <v-image
                 :config="{image: img_text_mono, x: 510, y: 170}"
                 ref="monoRef" 
                 @touchstart="iniciarLineaMono"
                >
                </v-image>
                <v-image
                 :config="{image: img_text_moto, x: 250, y: 170}"
                 ref="motoRef" 
                 @touchstart="iniciarLineaMoto"
                >
                </v-image>
                <v-image
                 :config="{image: img_text_mano, x: 370, y: 170}"
                 ref="manoRef"
                 @touchstart="iniciarLinea"
                >
                </v-image>
                <v-image
                 :config="{image: img_text_paleta, x: 0, y: 170}"
                 ref="paletaRef" 
                 @touchstart="iniciarLineaPaleta"
                >
                </v-image>
                <v-image
                  :config="{image: img_text_pelota, x: 125, y: 170}"
                  ref="pelotaRef" 
        ...
        methods: {
            ...
            observable(){
                ...
                if((vm.mano_seleccionado && vm.mono_seleccionado) &&(vm.moto_seleccionado && vm.paleta_seleccionado && vm.pelota_seleccionado)){
                    ...
                    vm.$router.push({ path: `/testprofundo2/`,
                    query: {
                        valor_ejercicio1:{
            ...
            trazarLineaMano(event){
                let vm = this;
                console.log("IMPRIMIENDO LINEA CON COORDENADAS");
                console.log(vm.configLineTxtPaleta);
                /* let coordLinePaleta     = vm.configLineTxtPaleta.attrs;
                let coordsLine          = vm.configLine.attrs;
                let coordsLineMono      = vm.configLineTxtMono.attrs;
                let coordsLineMoto      = vm.configLineTxtMoto.attrs;
                let coordsLinePelota    = vm.configLineTxtPelota.attrs;


                if(coordLinePaleta){
                    if(vm.configLineTxtPaleta.attrs.points[0] > 0 ){
                    ...
                    if(vm.configLine.attrs.points[0] > 0 ){
                        console.log("TRAZANDO LINEA MANO");
                        vm.mano_seleccionado = true; //La linea ha llegado a tocar la imagen 'mano' por tal razon es true, ha sido seleccionada.
                        vm.$refs.imgMano.getNode().off("touchend");
                    }
                }
                else if(coordsLineMono){
                    if(vm.configLineTxtMono.attrs.points[0] > 0){
                         console.log("TRAZANDO LINEA MANO");
                        vm.mano_seleccionado = true; //La linea ha llegado a tocar la imagen 'mano' por tal razon es true, ha sido seleccionada.
                        vm.$refs.imgMano.getNode().off("touchend");
                    }
                }
                else if(coordsLineMoto){
                    if(vm.configLineTxtMoto.attrs.points[0] > 0){
                        console.log("TRAZANDO LINEA MANO");
                        vm.mano_seleccionado = true; //La linea ha llegado a tocar la imagen 'mano' por tal razon es true, ha sido seleccionada.
                        vm.$refs.imgMano.getNode().off("touchend");
                    }
                }
                else if(coordsLinePelota){
                    if(vm.configLineTxtPelota.attrs.points[0] > 0){
                        console.log("TRAZANDO LINEA MANO");
                        vm.mano_seleccionado = true; //La linea ha llegado a tocar la imagen 'mano' por tal razon es true, ha sido seleccionada.
                        vm.$refs.imgMano.getNode().off("touchend");
                    }
